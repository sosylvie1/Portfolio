...
<body class="bg-gray-50" x-data="{ open: false }" x-cloak>
    <div class="min-h-screen md:flex">
        ...
        <aside
            ...
            :class="{ 'translate-x-0': open }" role="navigation" aria-label="Menu d’administration">
            ...
            <nav class="px-2 py-3 space-y-1">
                <a href="{{ route('admin.dashboard') }}" class="{{ $baseLink }} {{ $isActive('admin.dashboard') }}"
                    @click="open=false">📊 Dashboard</a>

                <a href="{{ route('admin.projets.index') }}"
                    class="{{ $baseLink }} {{ $isActive('admin.projets.*') }}" @click="open=false">📂 Projets</a>

                <a href="{{ route('admin.users.index') }}" class="{{ $baseLink }} {{ $isActive('admin.users.*') }}"
                    @click="open=false">👥 Visiteurs</a>

                <a href="{{ route('admin.contacts.index') }}"
                    class="{{ $baseLink }} {{ $isActive('admin.contacts.*') }}" @click="open=false">✉️
                    Messages</a>

                <div class="pt-2 mt-2 border-t">
                    <a href="{{ route('accueil') }}" class="{{ $baseLink }}" @click="open=false">🏠 Accueil
                        public</a>
                </div>
